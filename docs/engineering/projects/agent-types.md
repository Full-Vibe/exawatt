...
- **T2 Type format and launch application** — the documented file format and the launch path that applies a Type through the existing settings-injection seam, with capability requirements declared against ENG-003 source truth. AMENDED 2026-08-03 by ENG-016 D46 and its operator-confirmed lightweight-launcher review: naming a Launch Configuration saves only a friendly reusable preset; it does not make the launch a worker or promote it into a Type. T2 supplies the explicit portable identity, instructions, required tools, and capability requirements that a configuration may select through quiet Customize. T1's standalone composer icon retires into that entry using the same `announced` treatment. ENG-003 S2 also supplies the first real mechanism to declare against: `opencode --agent` is a named worker the source itself understands, so "Types declare what they require; sources declare what they can do" gets a second, non-Claude instance to be true about.
