...
- **T2 Type format and launch application** — the documented file format and the launch path that applies a Type through the existing settings-injection seam, with capability requirements declared against ENG-003 source truth. AMENDED 2026-08-03 by ENG-016 D46 (decision `0028`): a **named Launch Configuration is the on-ramp to a Type**. D46 makes harness + model + effort + Type one selectable thing in the composer; naming one is the moment a launch stops being a set of settings and starts being a worker, and T2 is what that named thing gains identity, instructions, and required tools from. T1's standalone composer icon retires into D46's Type axis — the same `announced` treatment, a contextually correct anchor. ENG-003 S2 also supplies the first real mechanism to declare against: `opencode --agent` is a named worker the source itself understands, so "Types declare what they require; sources declare what they can do" gets a second, non-Claude instance to be true about.
