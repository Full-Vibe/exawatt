...
- **One lifecycle vocabulary** (ENG-015 S6.4, 2026-09-23). A Session's lifecycle word (Paused, Exited, Interrupted, Closed, Resume failed, Resuming, Draft), its one-line ending, its tone, and the verb that answers it come from `packages/ui-model/src/session-lifecycle.ts` and nowhere else. A surface that needs to say what state a Session's process is in reads that owner; it never keeps a private ladder of words. Each word is spelled once, in `SESSION_LIFECYCLE_WORD` (`packages/core/src/session-lifecycle-words.ts`), because the native Session menu is built in Electron main, which reaches `@exawatt/core` and not `@exawatt/ui-model`; the command-verb manifest composes its resume labels from that table, ⌘K counts with the owner's `resumableAgentsNoun`, and the resume verbs' disabled reasons and chord announcements read `SESSION_RESUME_UNAVAILABLE`, so no verb names the Agents it acts on with a synonym ("parked") of its own. **Whether a Session can resume is the same derivation as its word** (BUG-185): `sessionCanResume` is the owner's verb, `tabCanResumeAsAgent` reads it, and every count, resume verb and refusal reads that. Paused promises resume, so only a Session that can resume wears it; a clean stop with no recorded conversation is Closed (its verb reconnects first), like a closed shell. A count keeps the word "paused" only while every Agent it counts prints Paused (a clean quit); a mixed set names the action instead ("3 Agents to resume"). How it ended includes a signal (BUG-186): a signalled death is Exited, `Ended by SIGKILL`, and an exit recorded before the signal was kept reads `Exit status not recorded`, never "Stopped cleanly". The line beside the word adds a fact (`Stopped cleanly · conversation kept`, `Exited with code 137 · conversation kept`) and never repeats the word. The status vocabularies (turn state, attention, status light) stay layered as `switcher-rows.ts` documents; this rung is about lifecycle only. `session-lifecycle-vocabulary.test.tsx` renders one Session through the tab strip, recovery bar, restore panel, paused record, and Team tile and pins the shared words, and pins the native menu, shortcut sheet and ⌘K resume labels to the word that Session's tab prints; `session-resume-truth.test.tsx` walks every combination of lifecycle facts through the strip and the bar and fails if a tab's word and the resume set disagree for any of them.
- **No em dashes in operator copy** (operator standing rule, 2026-08-13; recorded here 2026-09-23). Strings a user reads on a surface (labels, state lines, tooltips, accessible names, toasts) separate clauses with a middle dot, a comma, or a full stop, never an em dash. Code comments and engineering docs are out of scope. `pnpm copy:check` enforces it on every renderer landing (BUG-207): an em dash in JSX text, a string or template text that production can render fails, and a string that is only an em dash (the empty-value glyph in a cell) is allowed. Rewrite the sentence rather than swap in a comma.
...
| `/hud-gallery/design-canon` | **Keep** | the briefing surface for an external design partner: the kernel, the appearance contract, the board's composition doctrine, the measured site constraints, every external reference the canon cites, and what is deliberately unshaped. It is a RENDERING of the canonical docs and owns no decisions; when canon moves, `canon.ts` follows it. **Its budget is part of the artifact** (operator, 2026-08-17: the 4,400-word first version was unsendable): top-of-tree only, under 1,000 rendered words, pinned by a test. A rule earns a line only if a designer would build something different without it. Retire it when the design engagement no longer needs an entry point |
...
- 2026-09-24 — BUG-207: the **no em dashes in operator copy** rung gains
  ...
  scans the string content production can reach, decided by the import
  graph from the routes rather than a list of studies, and fails on an em
  dash. 65 strings were rewritten first, among them every live tab's
  tooltip, the ⌘W close confirmation and the Privacy table's destinations.
  One capped exception: the one-click roadmap launch prompt, which is
  addressed to the Agent. Adherence, now enforced.
- 2026-09-23 — ENG-015 S6.4 (BUG-185, BUG-186): the **one lifecycle
  vocabulary** rung now covers resumability and signals. "Paused" meant two
  sets: the tab chose its word from how the process ended, the counts and
  resume verbs chose their members by whether a conversation id was
  recorded, so a crash read "4 Agents paused" over four Interrupted tabs and
  an Agent with no recorded conversation read Paused while its chord said it
  was not. One owner derivation now decides both; a count says "paused" only
  when that is true of every member, otherwise "N Agents to resume"; an
  unresumable clean stop is Closed. A signalled exit is Exited with the
  signal named. Deliberate improvement.
- 2026-09-23 — ENG-015 S6.4 follow-up: the **one lifecycle vocabulary**
  rung reaches the command-verb manifest. The native menu read "Resume
  Parked Agents" and ⌘K "Resume 2 parked Agents in …" about exactly the
  Agents the recovery bar counts as paused (the same `tabCanResumeAsAgent`
  set, resumed by the same scope). Parked was never a distinct state: a
  clean quit and an operator Pause both record `stopped-clean`, which the
  owner already calls Paused; BUG-025's "parked is remembered, not
  forgotten" contrasts parked with FORGOTTEN in main's memory, not with
  paused. The words moved to `SESSION_LIFECYCLE_WORD` in `@exawatt/core` so
  main's menu can read them. Adherence, not a new rule.
