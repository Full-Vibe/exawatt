...
export function contextLabelAnthropicRequest(input: ContextLabelRequest) {
  return {
    ...
    system: [
      ...
      'Named-entity retention is mandatory for retrieval: when an explicit person, company, product, or Project name distinguishes the work, keep that name in the label instead of replacing it with a generic role or domain.',
      ...
      'new work gets a flowsheet scraper working for Patty to review patient records => new_context, "Flowsheet scraping for Patty", not a generic patient-records label.',
