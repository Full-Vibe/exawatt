    ...
    },
    {
      ...
      nodes: [
        ...
        },
        {
          ...
          summary:
            'Planned ENG-023 D8 adds disclosed environmental context and operation-specific delivery receipts (not yet implemented). One declaration contract plus Electron-main observations power Settings, launch, delegation, saved customer-hosted OpenClaw attachment, authoritative reconnect, coworker projection, and source-granted primary-conversation commands. Codex delegation uses its version-probed read-side app-server protocol, atomically reconciles verified children, isolates child and parent-activity read failures, separately publishes complete/partial/unavailable observation health to Agent Sources, and rejects stale connection observations. Claude Code delegation reads the harness census off every Stop/SubagentStop and shares the same reconcile. Non-Agent background tasks remain separate from children; one core busy predicate drives attention and all projections, authoritative censuses correct ambient bells, explicit human gates retain precedence, and silent monitors persist until the source withdraws them or the Session exits. Without non-Agent work, a reported child expires when its coverage lapses rather than latching until process exit; replay positions remain optional declared capabilities. A pure retained-history projection now normalizes bounded Codex/Claude records without IO or execution; production reading remains unwired.',
          layer: 'coordination',
          status: 'active-build',
          x: 572,
          y: 335,
        ...
        },
        {
          ...
          summary:
            'Distribution-gated authenticated general reports, label votes/corrections, the ⌘⇧F quick-capture bar with pre-captured screenshots, an honest unavailable state when no intake is configured, and the operator reinflation triage loop draining rows into canonical repo state.',
          layer: 'coordination',
          status: 'implemented',
          x: 365,
          y: 575,
