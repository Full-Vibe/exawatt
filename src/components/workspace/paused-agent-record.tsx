 ...
 *
 * How it ended and the verb that answers it are said ONCE, by the
 * `SessionRestorePanel` that sits over this record in production; the
 * record repeats the lifecycle word from the same shared vocabulary
 * (ENG-015 S6.4) and adds what only it knows: the task, the summary, and
 * how much output was saved.
 */
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-2xl flex-col gap-4">
        ...
        <section
          className="flex flex-col gap-2 border-t pt-3 text-chrome-meta"
          style={{ borderColor: HUD.strokeFaint, color: HUD.textDim }}
        >
          {(!meta || meta.exists) && (
            <p data-paused-history-meta>
              {!meta
          ...
          {lines === null ? (
            <div>
              ...
              {failed && (
                <p
                  role="status"
                  data-paused-history-unreadable
                  className="mt-2 text-chrome-meta"
                  style={{ color: HUD.amber }}
                >
                  Saved history unreadable
                </p>
              )}
            </div>
          ) : (
