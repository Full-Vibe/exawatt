...
function OpenCodeIcon({ size = 12 }: { size?: number }) {
  return (
    <svg
      data-slot="harness-glyph"
      viewBox="0 0 24 24"
...
function GrokIcon({ size = 12 }: { size?: number }) {
  return (
    <svg
      data-slot="harness-glyph"
      viewBox="0 0 24 24"
...
type SourceMarkComponent = ComponentType<{ size?: number }>;

/**
 * The one source-to-mark registry. Keyed by every declared Agent Source
 * (`contracts/agent-sources.json`), so a new source fails type-check here
 * until it names its mark, or says `null` on purpose: a source with no vendor
 * mark renders each surface's own fallback (Demo Mode is Exawatt's scenario
 * source, not a vendor).
 */
const SOURCE_MARKS = {
  claude: ClaudeIcon,
...
type MarkedSource = {
  [Source in keyof typeof SOURCE_MARKS]: (typeof SOURCE_MARKS)[Source] extends null
    ? never
    : Source;
}[keyof typeof SOURCE_MARKS];

/** Does this source have a brand mark? Accepts any string, because the site
 *  reads ids out of capture data. */
export function sourceMarkExists(source: string): source is MarkedSource {
  return (
    Object.hasOwn(SOURCE_MARKS, source) &&
    SOURCE_MARKS[source as keyof typeof SOURCE_MARKS] !== null
  );
}

/**
 * A source's brand mark, the one component every surface renders. Shell and
 * the coming-soon catalog entries have no mark; `fallback` is what the caller
 * shows then (nothing, by default).
 */
export function SourceGlyph({
  ...
  if (!sourceMarkExists(source)) return fallback;
  const Mark = SOURCE_MARKS[source];
  return <Mark size={size} />;
}

/** A PTY harness's mark; shell has none. */
